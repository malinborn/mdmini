...
**Ни одна из трёх верб не ходит в командный сокет.** Источник истины — файл,
значит `question`, `answer` и `watch` работают с файлами напрямую и являются
локальными офлайн-вербами, как существующие `help` и `agent`: та же ранняя ветка
в `run_ai_cli`, та же ветка без запуска приложения в `scripts/mdmini`. Сокет
остаётся только у `show`/`edit`/`ask`, которым нужно живое окно. Следствия:
вся фича работает при закрытом md-mini; в `ai_socket.rs` не появляется ни одной
новой команды протокола; а когда приложение всё же открыто, ответ агента
доезжает до UI обычным файловым watcher'ом.

Отдельная верба для доставки:

| Команда | Поведение |
|---|---|
| `mdmini watch [<dir>]` | Долгоживущий процесс: рекурсивно следит за `.mdmini_comments_*.md` в дереве каталогов (по умолчанию — cwd) и печатает одну строку на каждый **новый** `open` коммент. Предназначен для передачи в Monitor |

**`watch` не ходит в сокет и не требует запущенного md-mini.** Он читает те же
файлы, что и всё остальное, обычным файловым watcher'ом. Три следствия, и все
полезные: доставка работает, даже если приложение закрыто; скоуп задаётся деревом
каталогов, то есть совпадает с cwd агента без всякой логики скоупа; и логика
дедупликации тестируется юнит-тестом без GUI и без сокета.
...
sequenceDiagram
    ...
    participant D as дерево каталогов
    ...
    C->>D: рекурсивный watch<br/>на .mdmini_comments_*.md
    Note over C: ни сокета, ни запущенного app.<br/>stdout пуст, событий нет
...
| `src-tauri/src/ai_socket.rs` | Три новые **локальные** вербы (`question`, `answer`, `watch`) в `CliVerb`, `parse_cli_args` и ранней ветке `run_ai_cli`, плюс `USAGE`. Ни одной новой команды протокола сокета |
| `src-tauri/src/mcp_server.rs` | `question` и `answer` как MCP-tools, вызывающие `comments` напрямую — без сокета и без launch-if-not-running |
| `scripts/mdmini` | Три новые вербы в первую, офлайновую ветку — рядом с `help` и `agent` |
