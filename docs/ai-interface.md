...
### Or make it a skill instead

Both `mdmini agent` and `mdmini agent --mcp` print the block below after the snippet, and both "Connect AI via CLI" and "Teach your AI md-mini" in the AI menu carry it — an instruction file is re-read on every turn, so a block this long is context the agent pays for whether or not md-mini comes up. Kept verbatim in `SKILL_TIP` (`ai_socket.rs`); keep this fenced copy in sync with it.

```markdown
## Rather not carry this in context on every turn?
