...
### Two paths, and the skill one

Every surface that hands out a snippet — `mdmini agent`, `mdmini agent --mcp`, and "Connect AI via CLI" / "Teach your AI md-mini" in the AI menu — states both paths before the block (paste it, or make a skill of it and point at the skill from the instruction file) and closes with the block below. The reason is length: an instruction file is re-read on every turn, so a snippet this size is context the agent pays for whether or not md-mini comes up. Kept verbatim in `SKILL_TIP` (`ai_socket.rs`); keep this fenced copy in sync with it.

```markdown
## Making this a skill instead of a permanent block
